/**
 * Pure JS SHA-256 — same as net/sha256-js.ts (keep in sync).
 */
...
export function sha256BytesJs(data: Uint8Array): Uint8Array {
...
export function sha256HexJs(data: Uint8Array | string): string {
  const bytes = typeof data === "string" ? new TextEncoder().encode(data) : data;
  const hash = sha256BytesJs(bytes);
  let hex = "";
