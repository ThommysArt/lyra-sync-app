...
`android/` is generated by Expo prebuild (same as CI). Run prebuild once (or after plugin/config changes), then assemble.

| Command | Output |
|---------|--------|
| `pnpm run prebuild:native` | Generate `apps/native/android` (`expo prebuild`) |
...
```bash
pnpm run prebuild:native       # only when android/ is missing or config changed
