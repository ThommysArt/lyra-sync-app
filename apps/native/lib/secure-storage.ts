 ...
 */
function tryCreateSQLiteBulk(): (StorageLike & { hydrate: () => Promise<void>; flush?: () => Promise<void> }) | null {
  // Try expo-sqlite/kv-store first (simplest sync KV)
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const kvStore = require("expo-sqlite/kv-store") as {
      open?: (name: string) => { getItem: (k: string) => string | null; setItem: (k: string, v: string) => void; removeItem: (k: string) => void; getAllKeys?: () => string[] };
    };
    if (kvStore?.open) {
      const kv = kvStore.open("lyra");
      // Test that it works
        ...
        try {
          ...
          try {
            if (typeof kv.getAllKeys === "function") {
              keys = kv.getAllKeys().filter((k) => k.startsWith(STATE_PREFIX) || k === "lyra.v1.state");
            } else {
          ...
          if (keys.length === 0) {
            try {
              ...
              const lyraKeys = asyncKeys.filter((k) => k.startsWith(STATE_PREFIX) || k === "lyra.v1.state");
              let pairs: [string, string | null][] = [];
              if (lyraKeys.length === 0) {
                pairs = await AsyncStorage.multiGet(asyncKeys.filter((k) => k.startsWith("lyra.")));
              } else {
              ...
              if (keys.length > 0) console.info(`[lyra storage] migrated ${keys.length} keys from AsyncStorage to SQLite`);
            } catch (e) {
  ...
  try {
    ...
    const SQLite = require("expo-sqlite") as {
      openDatabaseSync?: (name: string) => {
        ...
        getAllSync: (sql: string, params?: unknown[]) => { key: string; value: string }[];
        getFirstSync: (sql: string, params?: unknown[]) => { value: string } | null;
      };
      openDatabase?: (...args: unknown[]) => unknown;
    };
    if (SQLite?.openDatabaseSync) {
      const db = SQLite.openDatabaseSync("lyra.db");
      db.execSync("CREATE TABLE IF NOT EXISTS kv (key TEXT PRIMARY KEY NOT NULL, value TEXT)");
      ...
      const doHydrate = async () => {
        try {
          ...
          try {
            rows = db.getAllSync("SELECT key, value FROM kv WHERE key LIKE 'lyra.%'");
          } catch {}
          if (rows.length === 0) {
            try {
              const asyncKeys = await AsyncStorage.getAllKeys();
              ...
              for (const [k, v] of pairs) {
                if (k && v != null) {
                  db.runSync("INSERT OR REPLACE INTO kv (key, value) VALUES (?, ?)", [k, v]);
                  cache.set(k, v);
                }
              }
              const state = await AsyncStorage.getItem("lyra.v1.state");
              if (state != null && !cache.has("lyra.v1.state")) {
                db.runSync("INSERT OR REPLACE INTO kv (key, value) VALUES (?, ?)", ["lyra.v1.state", state]);
                cache.set("lyra.v1.state", state);
              }
              const isolated = await AsyncStorage.getItem("lyra.v1.state.key");
              if (isolated != null && !cache.has("lyra.v1.state.key")) {
                db.runSync("INSERT OR REPLACE INTO kv (key, value) VALUES (?, ?)", ["lyra.v1.state.key", isolated]);
                cache.set("lyra.v1.state.key", isolated);
              }
              rows = db.getAllSync("SELECT key, value FROM kv WHERE key LIKE 'lyra.%'");
              if (rows.length > 0) console.info(`[lyra storage] migrated ${rows.length} keys from AsyncStorage to SQLite`);
            } catch (e) {
              console.warn("[lyra storage] AsyncStorage migration failed", e);
            }
          }
          for (const r of rows) if (r.key && r.value != null) cache.set(r.key, r.value);
          // Ensure critical keys
          for (const k of [STORAGE_KEY, `${STORAGE_KEY}.key`]) {
            if (!cache.has(k)) {
              try {
                const row = db.getFirstSync("SELECT value FROM kv WHERE key = ?", [k]);
 ...
 */
function createAsyncStorageBulk(): StorageLike & { hydrate: () => Promise<void>; flush: () => Promise<void> } {
  const cache = new Map<string, string>();
  let ready = false;
  let hydratePromise: Promise<void> | null = null;
  ...
  const enqueue = (op: Promise<void>): Promise<void> => {
    const p = op.catch((e) => {
      console.warn("[lyra storage] write failed", e instanceof Error ? e.message : String(e));
    });
    // Chain
...
export function createSecureLyraStorage(): SecureLyraStorage {
  ...
  let bulk: StorageLike & { hydrate?: () => Promise<void>; flush?: () => Promise<void> };
  ...
  if (canUseWebLocal) {
    ...
    if (sqlite) {
      ...
      console.info("[lyra storage] using AsyncStorage (fallback) — for durability add expo-sqlite");
...
export async function migratePrivateKeyToSecureStore(
  ...
  try {
    ...
    if (parsed.privateKey) {
      ...
      await Promise.resolve(storage.setItem(stateKey, JSON.stringify({ ...rest, privateKey: null })));
