/**
 * Config plugin: harden react-native-tcp-socket against
 * java.lang.IllegalArgumentException: No socket with id N
 *
 * The native module throws IllegalArgumentException when getTcpClient
 * is called for an id that is already closed / never created (e.g.
 * connect() failed before socketMap.put, or double destroy). That
 * exception runs on ThreadPoolExecutor and crashes the app because
 * it is uncaught.
 *
 * This plugin patches TcpSocketModule.java at prebuild time:
 *  - getTcpClient / getTcpServer return null + Log.w instead of throwing
 *  - write / end / destroy / close / pause / resume / setNoDelay / setKeepAlive
 *    check for null and ignore rather than throwing
 *  - connect() cleans up map on failure so destroy() doesn't race
 *  - end() removes id from map so second destroy is idempotent
 *
 * JS side hardening is also done in tcp-http-client.ts and
 * peer-server.native.ts (done flags, safeDestroy, etc.) but the
 * native fix is required to prevent the Java crash.
 */
...
function patchJavaFile(javaPath) {
  ...
  // Patch getTcpClient / getTcpServer to be nullable
  src = src.replace(
  ...
  // Patch setNoDelay, setKeepAlive, pause, resume
  src = src.replace(
  ...
  // Patch getPeerCertificate / getCertificate
  src = src.replace(
...
function withTcpSocketCrashFix(config) {
  return withDangerousMod(config, [
    "android",
    async (cfg) => {
      const projectRoot = cfg.modRequest.platformProjectRoot;
      // projectRoot is apps/native/android, need to find java file in node_modules
      // Try multiple locations: workspace root, apps/native/node_modules, pnpm store
      const candidates = [];

      // 1. Standard: projectRoot -> ../../node_modules
      candidates.push(
        path.join(projectRoot, "..", "..", "..", "node_modules", "react-native-tcp-socket", "android", "src", "main", "java", "com", "asterinet", "react", "tcpsocket", "TcpSocketModule.java")
      );
      // 2. pnpm .pnpm dir at workspace root
      candidates.push(
        path.join(cfg.modRequest.projectRoot, "..", "..", "node_modules", ".pnpm")
      );
      // 3. Direct require resolve
      try {
        const resolved = require.resolve("react-native-tcp-socket/package.json", { paths: [projectRoot] });
        candidates.push(path.join(path.dirname(resolved), "android", "src", "main", "java", "com", "asterinet", "react", "tcpsocket", "TcpSocketModule.java"));
      } catch {}
      try {
        const resolved = require.resolve("react-native-tcp-socket/package.json", { paths: [cfg.modRequest.projectRoot] });
        candidates.push(path.join(path.dirname(resolved), "android", "src", "main", "java", "com", "asterinet", "react", "tcpsocket", "TcpSocketModule.java"));
      } catch {}

      // Also try workspace root
      try {
        const wsRoot = path.join(projectRoot, "..", "..", "..");
        const resolved = require.resolve("react-native-tcp-socket/package.json", { paths: [wsRoot] });
        candidates.push(path.join(path.dirname(resolved), "android", "src", "main", "java", "com", "asterinet", "react", "tcpsocket", "TcpSocketModule.java"));
      } catch {}

      // Find pnpm store files
      const fsExtra = fs;
      let patched = false;
      for (const cand of candidates) {
        if (cand.endsWith("TcpSocketModule.java") && fsExtra.existsSync(cand)) {
          if (patchJavaFile(cand)) {
            console.log(`[with-tcp-socket-crash-fix] patched ${cand}`);
            patched = true;
          }
        } else if (cand.includes(".pnpm")) {
          // Search .pnpm for any TcpSocketModule.java
          try {
            const pnpmDir = cand;
            if (fsExtra.existsSync(pnpmDir)) {
              const entries = fsExtra.readdirSync(pnpmDir);
              for (const e of entries) {
                if (e.startsWith("react-native-tcp-socket@")) {
                  const javaPath = path.join(pnpmDir, e, "node_modules", "react-native-tcp-socket", "android", "src", "main", "java", "com", "asterinet", "react", "tcpsocket", "TcpSocketModule.java");
                  if (fsExtra.existsSync(javaPath) && patchJavaFile(javaPath)) {
                    console.log(`[with-tcp-socket-crash-fix] patched pnpm ${javaPath}`);
                    patched = true;
                  }
                  // Also check nested pnpm structure
                  const alt = path.join(pnpmDir, e, "node_modules", "react-native-tcp-socket", "android", "src", "main", "java", "com", "asterinet", "react", "tcpsocket", "TcpSocketModule.java");
                  if (fsExtra.existsSync(alt) && patchJavaFile(alt)) {
                    console.log(`[with-tcp-socket-crash-fix] patched pnpm alt ${alt}`);
                    patched = true;
                  }
                }
              }
              // Also brute find
              const findCmd = `find "${pnpmDir}" -name "TcpSocketModule.java" 2>/dev/null`;
              try {
                const { execSync } = require("node:child_process");
                const out = execSync(findCmd, { encoding: "utf8" });
                for (const p of out.split("\n").map(s=>s.trim()).filter(Boolean)) {
                  if (fsExtra.existsSync(p) && patchJavaFile(p)) {
                    console.log(`[with-tcp-socket-crash-fix] patched find ${p}`);
                    patched = true;
                  }
                }
              } catch {}
      ...
      if (!patched) {
        // Fallback: search whole workspace
        try {
          const { execSync } = require("node:child_process");
          const wsRoot = path.join(projectRoot, "..", "..", "..");
          const out = execSync(`find "${wsRoot}" -name "TcpSocketModule.java" 2>/dev/null | head -20`, { encoding: "utf8" });
          for (const p of out.split("\n").map(s=>s.trim()).filter(Boolean)) {
            if (patchJavaFile(p)) {
              console.log(`[with-tcp-socket-crash-fix] patched workspace find ${p}`);
              patched = true;
            }
          }
        } catch {}
      ...
      if (!patched) {
        console.warn("[with-tcp-socket-crash-fix] WARNING: could not locate TcpSocketModule.java to patch — crash may persist until manual patch");
      }
...
module.exports = createRunOncePlugin(withTcpSocketCrashFix, "with-tcp-socket-crash-fix", "1.0.0");
